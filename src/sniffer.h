...
#include <netinet/ether.h>  /* Transmission Control Protocol */
...
// convert between big-endian and little-endian values, 4-byte number
				...
				if (packet_type == END_SESSION_TYPE) {
					...
					if (packet_type == START_SESSION_TYPE) {
						if (SHOW_DISPLAY == 'y') {
							...
							printf("Terminal: %s\n", sess_data.terminal);
							printf("Program: %s\n", sess_data.program);
							printf("SID: %i\n",  endian_swap((unsigned int &)sess_data.sid));
							printf("Serial: %s\n", sess_data.serial);
							printf("Client_IP: %s\n", sess_data.client_ip);
...
int process_packet(struct transaction_packet *trans_packet, char *RecvBufSplit, char *timestamp) {
	...
	//end of communication socket >= end of Oracle Session
        if (((tcphdr->th_flags & TH_RST) == TH_RST) || ((tcphdr->th_flags & TH_FIN) == TH_FIN)) {
	...
	//  }//lentcp!=0
		...
		split = 0;
		current_split_packet = FindItem(first_split_packet, (unsigned char *) szSourceIP, tcphdr->th_sport);
		...
		if (current_split_packet != NULL) {
			...
			if (aux < pos && aux != -1) {
				pos = aux;
                        ...
                        aux = str_find_unsensitive(datatcp, lentcp, "create");
			if (pos == -1) {
				pos = aux;
			}
			if (aux < pos && aux != -1) {
				pos = aux;
                        ...
                        aux = str_find_unsensitive(datatcp, lentcp, "drop");
			if (pos == -1) {
				pos = aux;
			}
			if (aux < pos && aux != -1) {
		...
		if (pos != -1) {         
			...
			if (TARGET_DB_Oracle == 1) {
				...
				if (i == lentcp) {
					current_split_packet = AddItem(first_split_packet, (unsigned char *) szSourceIP, tcphdr->th_sport, dummy_char_count);
            ...
            if (TARGET_DB_MySQL == 1) {
                if (session_values_MySQL(datatcp, lentcp, &sess_data, szSourceIP, tcphdr->th_sport, szDestIP, tcphdr->th_dport, timestamp) == 2) {
...
void init_net() {
    ...
    if ((descr = pcap_open_live(linux_interface, PAKSIZE, 1, 1000, errbuf)) == NULL) {
        ERROR_MESSAGE(errbuf);
        exit(1);
    }

    // for MySQL we can compile a filter program
    if (TARGET_DB_MySQL == 1) {
        /* Look up info from the capture device. */
        if (pcap_lookupnet(linux_interface, &netaddr, &mask, errbuf) == -1) {
            ERROR_MESSAGE(errbuf);
            exit(1);
        }

        /* Compiles the filter expression into a BPF filter program */
        if (pcap_compile(descr, &filter, "tcp and port 3306", 1, mask) == -1) {
            ERROR_MESSAGE(pcap_geterr(descr));
            exit(1);
        }
...
int session_values_MySQL(unsigned char *datatcp, int lentcp, SESS *sess_data, char server_ip[], unsigned short int server_port, char client_ip[], unsigned short int client_port, char *timestamp) {
    ...
    if (datatcp[3] == 0 && (datatcp[2] != 0 || datatcp[1] != 0 || datatcp[0] != 0)) {
        result = 1;
        pos = 3;
        do {
            pos++;
        ...
        pos++;
        aux = 0;
        if (datatcp[pos + 3] != 0 || datatcp[pos + 2] != 0 || datatcp[pos + 1] != 0 || datatcp[pos] != 0) {
            do {
                sess_data->sid[3 - aux] = *(datatcp + pos + aux);
                aux++;
                if ((aux == 4) || (pos + aux == lentcp)) {
                    sess_data->sid[4] = 0;
                    aux = -1;
                }
            } while (aux > 0);
            // printf("thread_id: %.2x %.2x %.2x %.2x\n", sess_data->sid[0], sess_data->sid[1], sess_data->sid[2], sess_data->sid[3]);
            // printf("thread_id3: %i\n", endian_swap((unsigned int &)sess_data->sid));
    ...
    if (sess_data->sid[0] != 0 || sess_data->sid[1] != 0 || sess_data->sid[2] != 0 || sess_data->sid[3] != 0) {
        if (datatcp[3] == 1 && lentcp > 0) {
            ...
            if ((* (datatcp + pos + aux) != 0) && aux == 23) {
                ...
                aux = 0;
                do {
                    sess_data->dbname[aux] = *(datatcp + pos + aux);
                    aux++;
                } while (* (datatcp + pos + aux) > 0);
...
// Listens the socket and returns a MySQL packet
...
#define SIZEOF_ETHERNET_HEADER 14
    ...
    while ((res = pcap_next_ex(descr, &pkthdr, &packet)) >= 0) {
        ...
        if (res == 0) {
            ...
            //printf("Good Packet\n");
            iphdr = (struct ip *) (packet + SIZEOF_ETHERNET_HEADER);
            tcphdr = (struct tcphdr *) (packet + SIZEOF_ETHERNET_HEADER + (iphdr->ip_hl)*4); //iphdr->ip_hl gives the number of 32 bit words in the IP Header, so (iphdr->ip_hl)*4 gives the total size of the IP Header

            pIpheader = (struct ipheader *) (packet + SIZEOF_ETHERNET_HEADER);
            pTcpheader = (struct tcpheader *) (packet + SIZEOF_ETHERNET_HEADER + (iphdr->ip_hl)*4); //iphdr->ip_hl gives the number of 32 bit words in the IP Header, so (iphdr->ip_hl)*4 gives the total size of the IP Header

            //Using only TCP/IP packets
            //we can comment the next line because we have created a filter when we opened the packet capture from the network in init_net()
            // if (pcap_datalink(descr) == DLT_EN10MB && (tcphdr->th_off)*4 >= 20) { //Ethernet (10Mb, 100Mb, 1000Mb, and up) packet and size of TCP Header must be >= 20
            lentcp = (ntohs(iphdr->ip_len)- (sizeof (struct ip) + (tcphdr->th_off)*4)); //tcphdr->th_off gives the number of 32 bit words in the TCP Header, so (tcphdr->th_off)*4 gives the total size of the TCP Header
            /*
            printf("+-------------------------+\n");
            ...
            //Check Source IP
            strncpy(szSourceIP, inet_ntoa(iphdr->ip_src), 16);
            //Check Dest IP
            strncpy(szDestIP, inet_ntoa(iphdr->ip_dst), 16);

            datatcp = (u_char *) packet + SIZEOF_ETHERNET_HEADER + sizeof ( struct ip) + (tcphdr->th_off)*4; //tcphdr->th_off gives the number of 32 bit words in the TCP Header, so (tcphdr->th_off)*4 gives the total size of the TCP Header

            //If the source IP or the destination IP is from the client_ip defined in the config.cfg file or it is *
            if ((strcmp((char *) client_ip, szDestIP) == 0) || (strcmp((char *) client_ip, szSourceIP) == 0) || (client_ip[0] == '*')) {
                //With Windows Oracle Server the Oracle port is changed at startup of a connection
                //The server sends a packet containing the new port to be used by the client
                if (((strcmp((char *) DB_listener_ip, szSourceIP) == 0) || DB_listener_ip[0] == '*') && (DB_listener_port == ntohs(tcphdr->th_sport)) && (((tcphdr->th_flags & TH_RST) != TH_RST) && ((tcphdr->th_flags & TH_FIN) != TH_FIN))) {
                    //It is a packet from the server
                    //Test for the MySQL "Handshake Initialisation Packet"
                    //          pos=str_find_unsensitive ( datatcp,lentcp,"(port=" );
                    if (datatcp[3] == 0)//In the start of a MySQL communication the Server Packet Number is 0
                    {
                        ...
                        i = 0;
                        do {
                            if ((strcmp((char *) trans_packet_current->client_ip, szDestIP) == 0) && (trans_packet_current->client_port == tcphdr->th_dport)) {
                                break;
                            }
                            i++;
                            *trans_packet_current++;
                        ...
                        if (i == TRANSACTION_PACKET_SIZE) {
                            ...
                            do {
                                if (trans_packet_current->client_ip[0] == 0) {
                                    strcpy((char *) trans_packet_current->client_ip, szDestIP);
                                    //                                    trans_packet_current->client_port = '\0';
                                    trans_packet_current->client_port = tcphdr->th_dport;
                                    strcpy((char *) trans_packet_current->server_ip, szSourceIP);
                                    //                                  trans_packet_current->server_port = htons(int_port);
                                    trans_packet_current->server_port = tcphdr->th_sport;
                                    trans_packet_current->server_XID = 0;
                                    trans_packet_current->client_XID = 0;
                                    trans_packet_current->end_transaction = 'n';
                                    break;
                    ...
                    //It is not a packet from the server let's see if it is one of the sessions that are already started
                {
                    ...
                    if ((ntohs(tcphdr->th_sport) == DB_listener_port || ntohs(tcphdr->th_dport) == DB_listener_port)
                            &&
                            ((strcmp(szSourceIP, (char *) DB_listener_ip) == 0 || strcmp(szDestIP, (char *) DB_listener_ip) == 0) || DB_listener_ip[0] == '*')) {
                        //do nothing
                    } else {
                        ...
                        do {
                            if (trans_packet_current->client_port == '\0') {
                                if (
                                        (((strcmp((char *) trans_packet_current->server_ip, szSourceIP) == 0) && (trans_packet_current->server_port == tcphdr->th_sport)))
                                        ||
                                        (((strcmp((char *) trans_packet_current->server_ip, szDestIP) == 0) && (trans_packet_current->server_port == tcphdr->th_dport)))
                                        ) {
                                    trans_packet_current->client_port = tcphdr->th_sport;
                                    break;
                                }
                            } else {
                                if (
                                        (((strcmp((char *) trans_packet_current->client_ip, szDestIP) == 0) && (trans_packet_current->client_port == tcphdr->th_dport))
                                        &&
                                        ((strcmp((char *) trans_packet_current->server_ip, szSourceIP) == 0) && (trans_packet_current->server_port == tcphdr->th_sport)))
                                        ||
                                        (((strcmp((char *) trans_packet_current->client_ip, szSourceIP) == 0) && (trans_packet_current->client_port == tcphdr->th_sport))
                                        &&
                                        ((strcmp((char *) trans_packet_current->server_ip, szDestIP) == 0) && (trans_packet_current->server_port == tcphdr->th_dport)))
                                        ) {
                                    break;
                ...
                //then the packet can be processed
                if (
                        ((ntohs(tcphdr->th_sport) == DB_listener_port || ntohs(tcphdr->th_dport) == DB_listener_port)
                        &&
                        ((strcmp(szSourceIP, (char *) DB_listener_ip) == 0 || strcmp(szDestIP, (char *) DB_listener_ip) == 0) || DB_listener_ip[0] == '*'))
                        ||
                        (i < TRANSACTION_PACKET_SIZE)
